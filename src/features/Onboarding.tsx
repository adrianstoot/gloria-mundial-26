...
import { getOfficialSquad } from '../data'
  ...
  return (
    <main className="onboarding">
      ...
      <div className="onboarding__steps" aria-label={`Paso ${step} de 4`}>
        {[1, 2, 3, 4].map((item) => <span key={item} className={item <= step ? 'is-active' : ''}><i>{item < step ? <Check size={12} /> : item}</i><b>{item === 1 ? 'Seleccionador' : item === 2 ? 'Tu selección' : item === 3 ? 'Sorteo' : 'Preparación'}</b></span>)}
      </div>
      {children}
    </main>
  )
...
export function ManagerSetup() {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    // Split coach name into name and surname
    const [firstName, ...lastNameParts] = coach.name.split(' ')
    const lastName = lastNameParts.join(' ')

    // Apply specialization effects to starting metrics
    const spec = profile.specialization || 'tactica'
    let additionalModifiers = {}
    if (spec === 'tactica') {
      additionalModifiers = { tacticalFamiliarity: Math.min(100, (adjusted.tacticalFamiliarity || 0) + 10) }
    } else if (spec === 'gestion') {
      additionalModifiers = { cohesion: Math.min(100, (adjusted.cohesion || 0) + 10) }
    } else if (spec === 'impulso') {
      additionalModifiers = { morale: Math.min(100, (adjusted.morale || 0) + 10) }
    }
    ...
    <OnboardingFrame step={1} onBack={() => navigate('/')}>
      <section className="setup-page">
        ...
        >
          ...
          <div className="coach-carousel">
            ...
            <article className="coach-card" aria-live="polite">
              <img
                src={coach.visual}
                alt={`Retrato ficticio de ${coach.name}`}
                style={{ objectPosition: coach.objectPosition || 'center 18%' }}
              />
          ...
          <div className="coach-indicators">
            {coach.indicators.map((indicator) => {
              const spec = profile.specialization || 'tactica'
              const isBoosted =
                (indicator.label === 'Táctica' && spec === 'tactica') ||
                (indicator.label === 'Gestión' && spec === 'gestion') ||
                (indicator.label === 'Impulso' && spec === 'impulso')
              const displayValue = isBoosted ? Math.min(100, indicator.value + 10) : indicator.value
              return (
                <span key={indicator.label} style={{ boxShadow: isBoosted ? 'inset 0 0 0 1px var(--selection-gold)' : undefined }}>
                  <small>{indicator.label}</small>
                  <b>{displayValue}{isBoosted && <span style={{ color: 'var(--selection-gold)', fontSize: '11px', marginLeft: '4px' }}>+10</span>}</b>
                  <i><em style={{ width: `${displayValue}%`, background: isBoosted ? 'var(--selection-gold)' : undefined }} /></i>
                </span>
              )
            })}
          </div>
        ...
        <form className="setup-form panel" onSubmit={submit}>
          <header className="setup-form__title"><span className="eyebrow"><UserRound /> PERFIL DEL SELECCIONADOR</span><h2>Firma tu <em>historia</em></h2><p>Tu especialización, experiencia y filosofía definirán cómo te recibe el vestuario.</p></header>
          <div className="form-heading"><span>01</span><div><h2>Especialización</h2><p>Potencia uno de tus atributos clave como seleccionador.</p></div></div>
          <div className="experience-grid">
            {([
              ['tactica', BrainCircuit, 'Especialista Táctico', 'Añade +10 a Táctica y mejora el entendimiento táctico inicial.'],
              ['gestion', HeartHandshake, 'Gestor de Grupo', 'Añade +10 a Gestión y facilita la cohesión del vestuario.'],
              ['impulso', Sparkles, 'Líder Motivador', 'Añade +10 a Impulso y acelera la moral inicial del plantel.'],
            ] as const).map(([value, Icon, title, text]) => {
              const spec = profile.specialization || 'tactica'
              return (
                <button type="button" key={value} className={spec === value ? 'is-selected' : ''} onClick={() => setProfile({ ...profile, specialization: value })} style={{ minHeight: '94px' }}>
                  <Icon /><b>{title}</b><small>{text}</small>{spec === value && <Check className="experience-grid__check" size={15} />}
                </button>
              )
            })}
          </div>
          ...
          <div className="setup-form__footer"><span>{valid ? <><Check /> Perfil listo · {coach.name}</> : 'Completa especialización'}</span><button className="button button--gold" disabled={!valid}>ELEGIR SELECCIÓN <ArrowRight /></button></div>
        </form>
      </section>
    </OnboardingFrame>
  )
...
export function NationSelect() {
  const navigate = useNavigate()
  const { campaign, updateCampaign } = useGame()
  const defaultPlayable = uiNations.find((nation) => nation.code === 'MAR') ?? uiNations.find((nation) => PLAYABLE_NATION_CODE_SET.has(nation.code))
  ...
  const confirm = () => {
    ...
    updateCampaign({
      ...
      difficulty,
      squadIds: getOfficialSquad(selected.id).map((player) => player.id),
  ...
  return (
    <OnboardingFrame step={2} onBack={() => navigate('/crear-seleccionador')}>
      <section className="nation-page">
        <header className="nation-page__title"><span className="eyebrow"><Globe2 size={14} /> 48 CAMINOS HACIA LA GLORIA</span><h1>Elige la nación que <em>hará historia</em></h1></header>
        <div className="nation-page__layout">
                ...
                {[0].map((offset) => {
                  ...
                  return (
                    <button key={`${nation.id}-${offset}`} role="option" aria-selected={active} className={`nation-choice nation-choice--${offset < 0 ? 'previous' : offset > 0 ? 'next' : 'active'} ${active ? 'is-selected' : ''}`} onClick={() => active ? undefined : setSelectedId(nation.id)} title={`Dirigir a ${nation.name}`}>
                      ...
                      <span className="nation-choice__copy"><small>#{nation.worldRanking} DEL MUNDO</small><b>{nation.name}</b>{active && <em>{nation.style}</em>}</span>
          ...
          {selected && (
            <aside className="nation-detail" style={{ '--team-primary': selected.primaryColor, '--team-secondary': selected.secondaryColor } as React.CSSProperties}>
              <div className="nation-detail__hero"><span className={`fi fi-${selected.flagCode}`} /><div className="nation-detail__halo" /><small>#{selected.worldRanking} DEL MUNDO</small><h2>{selected.name}</h2><p>{selected.confederation}</p></div>
              ...
              <footer className="nation-detail__footer"><span className={selectionError ? 'is-error' : ''}>{selectionError || <><HeartHandshake /> {selected.name} espera tu decisión</>}</span><button className="button button--gold button--wide" onClick={confirm}>CONTINUAR AL SORTEO <ChevronRight /></button></footer>
            </aside>
          )}
        </div>
      </section>
